...
class infiltration(category2d):
	heading = '[INFILTRATION]'
	def __init__(self,linelist):
		pnames = ['Subcatchment', 'Suction', 'Ksat', 'IMD']
		category2d.__init__(self,linelist,pnames)
...
class swmm_model:
	...
	def lidChangeArea(self,subcat,lidname,newarea,DtoG):
		...
		acre = 43560.0
		new_lid_acre = newarea/acre #newarea: surface area of a standard GI unit 
		...
		lid_type = lid.type
		old_lid_area_str = lid_usage_class.get((subcat,lidname),'Area')  # LID area in SQUARE FEET
		old_lid_area = float(old_lid_area_str)
		old_lid_acre = old_lid_area/acre  # lid area in acre
		lid_number_str = lid_usage_class.get((subcat,lidname),'Number') # number of GI installation - from YAML
		lid_number = int(lid_number_str)
		
		subcatchments_class = self.moddict[subcatchments]
		subcat_area_str = subcatchments_class.get(subcat,'Area')
		subcat_area = float(subcat_area_str)  # Subcatchment area in acre
		subcat_PctImperv_old_str = subcatchments_class.get(subcat,'PctImperv') # get the value of PctImperv
...
####### the new lid installation and old installation		  
		lidOnPervList = ['BC','RG','IT','PP','RB','GR']  # fw added 'BC' and remove 'RB'
		if lid_type in lidOnPervList:  # LID displaces some of the subcatchment's imperv. area
			...
			subcatchments_class.change(subcat,'PctImperv',subcat_PctImperv_new_str)
		...
		lid_usage_class.change((subcat,lidname),'Area',newarea_str)  # change the number
		# adjust the FromImp parameter - the % of subcat area treated by each LID:
		#print "imperv_area = %s" % imperv_area
#		DR =impervious treated/GI area
		imperv_area_treated = lid_number*new_lid_acre*DtoG  #percImperv treated (ac)
		if imperv_area <= 0.0:
			lidFromImp = 0.0;
		else:
		...
		self.lidChangeFromImp(subcat,lidname,lidFromImp_new)

	def lidChangeNumber(self,subcat,lidname,newnumber):
		...
		lid_usage_class = self.moddict[lid_usage]
		lid_area_str = lid_usage_class.get((subcat,lidname),'Area')  # LID area in SQUARE FEET
		lid_area = float(lid_area_str)
		lid_acre = lid_area/acre
		lid_number_old_str = lid_usage_class.get((subcat,lidname),'Number')
		lid_number_old = int(lid_number_old_str)
		# Find out what LID type we are changing the area of:
		lid_controls_class = self.moddict[lid_controls]
		lid = lid_controls_class.liddict[lidname]
		lid_type = lid.type	
  ## fw added 'BC' and remove 'RB'
		lidOnPervList = ['BC','RG','IT', 'PP','GR','RB']
		if lid_type in lidOnPervList:  # LID displaces some of the subcatchment's imperv. area
			# so we must adjust the % Impervious parameter in the subcatchment:
			subcatchments_class = self.moddict[subcatchments]
			subcat_area_str = subcatchments_class.get(subcat,'Area')
			subcat_area = float(subcat_area_str)  # Subcatchment area in acre
			subcat_PctImperv_old_str = subcatchments_class.get(subcat,'PctImperv')
			subcat_PctImperv_old = float(subcat_PctImperv_old_str)


			imperv_area_old = subcat_area*(subcat_PctImperv_old/100.0)   # old impervious area in acre
		...
		lid_usage_class.change((subcat,lidname),'Number',newnumber_str)  # change the number


  
	def lidChangeFromImp(self,subcat,lidname,newnumber):
		# Note: there appear to be no interlinked calculations required
		# So just change the number:
		newnumber_str = "%0.3f" % newnumber
		lid_usage_class = self.moddict[lid_usage]
		lid_usage_class.change((subcat,lidname),'FromImp',newnumber_str)  # change the number

	def lidChangeWidth(self,subcat,lidname,newnumber):
		# Note: there appear to be no interlinked calculations required
		# So just change the number:
		newnumber_str = "%0.3f" % newnumber
		lid_usage_class = self.moddict[lid_usage]
		lid_usage_class.change((subcat,lidname),'Width',newnumber_str)  # change the number
        
	def lidGetArea(self,subcat,lidname):
		lid_usage_class = self.moddict[lid_usage]
		lid_area_str = lid_usage_class.get((subcat,lidname),'Area')  # LID area in SQUARE FEET
		lid_area = float(lid_area_str)	
		return lid_area		
    
	def lidControl(self,lidname,layer,param,newnumber):
		# Note: there appear to be no interlinked calculations required
		# So just change the number:
 		newnumber_str = "%0.3f" % newnumber
		lid_controls_class = self.moddict[lid_controls]
		lid_controls_class.change3d(lidname,layer,param,newnumber_str)  # change the number

#    
	def CatChangeKsat(self,subcat,newnumber):
		# Chage the satruated hydrolic conductivity ksat
 		newnumber_str = str(newnumber)
		infiltration_class = self.moddict[infiltration]
		infiltration_class.change(subcat,'Ksat',newnumber_str)  # change the number
#        
#	def lidChangeBernH(self,lidname,layer,param,newnumber):
#		# Chage the satruated hydrolic conductivity ksat
# 		newnumber_str = "%0.3f" % newnumber
#		lid_controls_class = self.moddict[lid_controls]
#		lid_controls_class.change3d(lidname,layer,'StorHt',newnumber_str)  # change the number       
